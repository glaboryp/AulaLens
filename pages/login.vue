<template>
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
    <UCard class="w-full max-w-md">
      <template #header>
        <div class="text-center">
          <img src="/logo-bg.png" alt="AulaLens Logo" class="w-24 h-24 mx-auto mb-4" />
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
            Bienvenido a AulaLens
          </h1>
          <p class="text-gray-600 dark:text-gray-400 mt-2">
            Una lente de claridad para Google Classroom
          </p>
        </div>
      </template>

      <div class="space-y-6">
        <div class="text-center">
          <p class="text-sm text-gray-600 dark:text-gray-400">
            Inicia sesión con tu cuenta de Google para acceder a tu dashboard personalizado
          </p>
        </div>

        <UButton
          @click="signIn('google')"
          color="white"
          variant="solid"
          size="xl"
          :loading="loading"
          block
          class="justify-center"
        >
          <template #leading>
            <svg class="w-5 h-5" viewBox="0 0 24 24">
              <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
          </template>
          Iniciar Sesión con Google
        </UButton>

        <div class="text-center">
          <p class="text-xs text-gray-500 dark:text-gray-400">
            Al iniciar sesión, aceptas nuestros términos de servicio y política de privacidad
          </p>
        </div>
      </div>
    </UCard>
  </div>
</template>

<script setup lang="ts">
const { signIn, data: session, status } = useAuth()
const loading = ref(false)

// Configurar meta tags para la página
useHead({
  title: 'Iniciar Sesión - AulaLens',
  meta: [
    { name: 'description', content: 'Inicia sesión en AulaLens con tu cuenta de Google para acceder a tu dashboard de Google Classroom.' }
  ]
})

// Redireccionar automáticamente si ya está autenticado
watchEffect(() => {
  if (status.value === 'authenticated') {
    navigateTo('/dashboard')
  }
})
</script>
